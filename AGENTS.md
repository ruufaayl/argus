...
Agent entry point for Argus. Read this first, then follow links for depth.
...
│   └── argus/       # Domain handlers (mirrors proto service structure)
├── proto/                  # Protobuf definitions (sebuf framework)
│   ├── buf.yaml            # Buf configuration
│   └── argus/       # Service definitions with HTTP annotations
...
- `server/argus/<domain>/` has RPC handlers matching proto services
...
1. Define proto message in `proto/argus/<domain>/`
2. Add RPC with `(sebuf.http.config)` annotation
3. Run `make generate`
4. Create handler in `server/argus/<domain>/`
