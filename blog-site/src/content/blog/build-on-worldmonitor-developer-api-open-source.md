---
title: "Build on ARGUS: Open APIs, Proto-First Architecture, and the Developer Platform"
description: "Build intelligence apps on ARGUS's typed API: 22 services, 92 proto files, 60+ edge functions, and auto-generated TypeScript clients. AGPL-3.0."
metaTitle: "Developer API & Open Source Platform | ARGUS"
...
heroImage: "/blog/images/blog/build-on-argus-developer-api-open-source.jpg"
pubDate: "2026-03-09"
---
...
ARGUS is designed differently. The entire intelligence platform, every data feed, every scoring algorithm, every aggregation pipeline, is built on a **typed API layer** that developers can use, extend, and build upon.

## Proto-First Architecture

ARGUS uses **Protocol Buffers (protobuf)** as the single source of truth for all API contracts. The codebase contains:
...
For developers building on ARGUS, this means you can trust the API contracts completely. If the proto says a field is `int64`, it's `int64`. If it says `repeated string`, it's an array of strings.

## 22 Service Domains

ARGUS's API is organized into domain-specific services:
...
```
api.argus.app/api/{domain}/v1/{rpc}
```

For example:

- `api.argus.app/api/market/v1/quotes` for stock quotes
- `api.argus.app/api/conflict/v1/events` for conflict data
- `api.argus.app/api/intelligence/v1/cii` for Country Instability Index scores

## Building with ARGUS's API
...
Feed ARGUS data into your own analytics:
...
- Build custom scoring models using ARGUS's raw data feeds

### Mobile Apps

Build a mobile app that consumes ARGUS's API for a custom mobile intelligence experience. The OpenAPI spec makes it accessible from any language (Swift, Kotlin, Python, Go).
...
ARGUS is AGPL-3.0. You can self-host the entire platform, including [local AI capabilities that run without cloud dependencies](/blog/posts/ai-powered-intelligence-without-the-cloud/):
...
For reference, ARGUS is built with:
...
The intelligence industry has a consolidation problem. A handful of vendors control the data, the algorithms, and the interfaces. Analysts are locked into ecosystems they can't customize, audit, or extend. See how ARGUS [compares to traditional intelligence tools](/blog/posts/argus-vs-traditional-intelligence-tools/) in practice.

ARGUS's open, typed, proto-first architecture is the alternative:
...
The intelligence platform of the future isn't a product. It's an ecosystem. ARGUS is building the foundation.

## Frequently Asked Questions

**Is the ARGUS API free to use?**
Yes. ARGUS is AGPL-3.0 open source. You can use the public API at api.argus.app or self-host the entire stack. There are no API keys required for public endpoints and no usage fees.
...
**Start building at [github.com/ruufaayl/argus](https://github.com/ruufaayl/argus). 22 services, 92 proto files, and a global intelligence dataset waiting for your application.**
