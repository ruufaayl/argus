...
Run the full ARGUS stack locally with Docker/Podman.
...
*/30 * * * * cd /path/to/argus && ./scripts/run-seeders.sh >> /tmp/wm-seeders.log 2>&1
```
...
|-----------|---------|------|
| `argus` | nginx + Node.js API (supervisord) | 3000 → 8080 |
| `argus-redis` | Data store | 6379 (internal) |
| `argus-redis-rest` | Upstash-compatible REST proxy | 8079 |
| `argus-ais-relay` | Live vessel tracking WebSocket | 3004 (internal) |
...
# Full Docker image
docker build -t argus:latest -f Dockerfile .
...
| 🔴 nginx won't start | Check `podman logs argus` — likely missing `gettext` package |
...
| 🚢 No vessel data | Set `AISSTREAM_API_KEY` in both `argus` and `ais-relay` services |
