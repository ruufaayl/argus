...
Thank you for your interest in contributing to ARGUS! This project thrives on community contributions — whether it's code, data sources, documentation, or bug reports.
...
ARGUS is a real-time OSINT dashboard built with **Vanilla TypeScript** (no UI framework), **MapLibre GL + deck.gl** for map rendering, and a custom Proto-first RPC framework called **Sebuf** for all API communication.
...
- **Internationalization** — help make ARGUS available in more languages
...
- Sebuf handler implementations go in `server/argus/{domain}/v1/`
...
- Proto definitions go in `proto/argus/{domain}/v1/`
...
1. **Proto definitions** in `proto/argus/{domain}/v1/` define services and messages
...
3. **Handlers** in `server/argus/{domain}/v1/handler.ts` implement the service interface
...
2. **Add the proto service** (if the data needs a backend proxy) — define messages and RPC methods in `proto/argus/{domain}/v1/`
3. **Generate stubs** — run `make generate`
4. **Implement the handler** in `server/argus/{domain}/v1/`
...
rclone copy public/data/country-boundary-overrides.geojson r2:argus-maps/
```
...
Use the [Bug Report issue template](https://github.com/ruufaayl/argus/issues/new/choose) when available.
...
Use the [Feature Request issue template](https://github.com/ruufaayl/argus/issues/new/choose) when available.
