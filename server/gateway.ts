...
import type { ServerOptions } from '../src/generated/server/argus/seismology/v1/service_server';
...
// Vary: Origin) does NOT cache these responses. CF sits in front of api.argus-intel.vercel.app
// and would otherwise pin ACAO: argus-intel.vercel.app on the cached response, breaking CORS
  ...
  return async function handler(originalRequest: Request): Promise<Response> {
    ...
    if (response.status === 200 && request.method === 'GET' && response.body) {
      ...
      if (mergedHeaders.get('X-No-Cache') || isUpstreamUnavailable) {
        ...
        // Only allow Vercel CDN caching for trusted origins (argus-intel.vercel.app, Vercel previews,
