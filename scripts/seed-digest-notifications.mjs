...
const RESEND_FROM = process.env.RESEND_FROM_EMAIL ?? 'Argus <alerts@argus-intel.vercel.app>';
...
function formatDigest(stories, nowMs) {
  ...
  const lines = [`Argus Daily Digest — ${dateStr}`, ''];
  ...
  lines.push('View full dashboard \u2192 argus-intel.vercel.app');
  return lines.join('\n');
}
...
function formatDigestHtml(stories, nowMs) {
  ...
  <div style="padding: 40px 32px 0;">
    ...
    <div style="text-align: center; margin-bottom: 36px;">
      <a href="https://argus-intel.vercel.app" style="display: inline-block; background: #4ade80; color: #0a0a0a; padding: 14px 36px; text-decoration: none; font-weight: 800; font-size: 13px; text-transform: uppercase; letter-spacing: 1.5px; border-radius: 2px;">View Full Dashboard</a>
    </div>
  </div>
  <div style="border-top: 1px solid #1a1a1a; padding: 24px 32px; text-align: center;">
    <div style="margin-bottom: 16px;">
      ...
      <a href="https://argus-intel.vercel.app" style="color: #666; text-decoration: none; font-size: 12px; margin: 0 12px;">argus-intel.vercel.app</a>
    </div>
    <p style="font-size: 11px; color: #444; margin: 0; line-height: 1.6;">
      ARGUS &mdash; Real-time intelligence for a connected world.<br />
      <a href="https://argus-intel.vercel.app" style="color: #4ade80; text-decoration: none;">argus-intel.vercel.app</a>
    </p>
  </div>
</div>`;
}
...
async function sendTelegram(userId, chatId, text) {
  ...
  const res = await fetch(
    ...
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'User-Agent': 'argus-digest/1.0' },
...
async function main() {
  ...
  for (const rule of rules) {
    ...
    const subject = `Argus Digest — ${shortDate}`;
