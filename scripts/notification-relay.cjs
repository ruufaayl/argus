...
async function upstashRest(...args) {
  const res = await fetch(`${UPSTASH_URL}/${args.map(encodeURIComponent).join('/')}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'User-Agent': 'argus-relay/1.0' },
  });
  if (!res.ok) {
...
async function drainHeldForUser(userId, variant, allowedChannelTypes) {
  ...
  const lines = [`Argus — ${events.length} held alert${events.length !== 1 ? 's' : ''} from quiet hours`, ''];
  ...
  const subject = `Argus — ${events.length} held alert${events.length !== 1 ? 's' : ''}`;

  let channels = [];
  try {
    const chRes = await fetch(`${CONVEX_SITE_URL}/relay/channels`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${RELAY_SECRET}`, 'User-Agent': 'argus-relay/1.0' },
...
async function sendTelegram(userId, chatId, text) {
  ...
  const res = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'User-Agent': 'argus-relay/1.0' },
...
async function processWelcome(event) {
  ...
  const text = `✅ Argus connected! You'll receive breaking news alerts here.`;
  if (channelType === 'slack' && ch.webhookEnvelope) {
    ...
    await sendEmail(ch.email, 'Argus Notifications Connected', text);
  }
}
...
async function processEvent(event) {
  ...
  const subject = `Argus Alert: ${event.payload?.title ?? event.eventType}`;
