...
- **`server/argus/intelligence/v1/deduct-situation.ts:52`** — `systemAppend: framework || undefined` → goes through weak filter
- **`server/argus/intelligence/v1/get-country-intel-brief.ts:99`** — same issue
- **`server/argus/news/v1/summarize-article.ts:127`** — correctly uses `sanitizeForPrompt(systemAppend)` before prompt build
...
## Technical Details
- Files: `server/_shared/llm.ts`, `server/_shared/llm-sanitize.js`, `server/argus/intelligence/v1/deduct-situation.ts:52`, `server/argus/intelligence/v1/get-country-intel-brief.ts:99`
