...
`SIMULATION_OUTCOME_LATEST_KEY = 'forecast:simulation-outcome:latest'` is defined independently in both `server/argus/forecast/v1/get-simulation-outcome.ts` and `scripts/seed-forecasts.mjs`. The same duplication exists for `SIMULATION_PACKAGE_LATEST_KEY`. `server/_shared/cache-keys.ts` (referenced in the argus-bootstrap-registration pattern) exists for exactly this purpose: shared Redis key constants that TypeScript handlers and seed scripts need to agree on. A future rename in one file without the other produces a silent miss where the handler reads an empty key forever.

## Findings

**F-1:**
```typescript
// server/argus/forecast/v1/get-simulation-outcome.ts line 10
...
// server/argus/forecast/v1/get-simulation-outcome.ts — replace local const:
...
- Files: `server/argus/forecast/v1/get-simulation-outcome.ts:10`, `server/argus/forecast/v1/get-simulation-package.ts:~10`, `server/_shared/cache-keys.ts`
