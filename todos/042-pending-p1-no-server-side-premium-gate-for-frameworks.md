...
## Findings
- **`server/argus/intelligence/v1/get-country-intel-brief.ts`** — reads `req.framework`, truncates to 2000 chars, passes to `callLlm` as `systemAppend`. No entitlement check.
- **`server/argus/intelligence/v1/deduct-situation.ts`** — same pattern, no entitlement check.
- **`server/argus/news/v1/summarize-article.ts`** — reads `req.systemAppend`, no entitlement check.
...
- Flagged by: security-sentinel, architecture-strategist, learnings-researcher (argus-pro-panel-gating skill)
...
## Technical Details
- Files: `server/argus/intelligence/v1/get-country-intel-brief.ts`, `server/argus/intelligence/v1/deduct-situation.ts`, `server/argus/news/v1/summarize-article.ts`
- PR: ruufaayl/argus#2380
- Reference: argus-pro-panel-gating skill (4-layer checklist)
