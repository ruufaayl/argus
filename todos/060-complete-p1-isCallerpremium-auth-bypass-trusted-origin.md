...
## Findings
- **`server/_shared/premium-check.ts:10-12`** — `if (keyCheck.valid) return true;` triggers for all argus.app sessions
- **`api/_api-key.js:49-68`** — `isTrustedBrowserOrigin()` returns `true` for `*.argus.app`, `*vercel.app`, `localhost`; causes `validateApiKey` to return `{ valid: true, required: false }` with no key present
...
## Acceptance Criteria
- [ ] Free-tier users on argus.app do NOT pass `isCallerPremium` check
