...
const headerKey = req.headers.get('X-Argus-Key') ?? '';
...
**Affected files:**
- `api/mcp.ts:440-451` — remove `urlKey` and change `const candidateKey = urlKey || headerKey` to `const candidateKey = req.headers.get('X-Argus-Key') ?? ''`

## Acceptance Criteria

- [ ] `?key=` URL parameter is not present in `api/mcp.ts` auth chain
- [ ] `X-Argus-Key` header path still works
