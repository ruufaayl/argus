...
if (bearerHeader?.startsWith('Bearer ') && !bearerApiKey) {
  return new Response(
    ...
    { status: 401, headers: { 'WWW-Authenticate': 'Bearer realm="argus", error="invalid_token"', ...corsHeaders } }
  );
}
```
...
- [ ] HTTP 401 response includes `WWW-Authenticate: Bearer realm="argus", error="invalid_token"`
