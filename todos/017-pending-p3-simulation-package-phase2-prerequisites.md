...
- There is no `getSimulationPackage(runId)` RPC handler in `server/argus/forecast/v1/`
...
Create `server/argus/forecast/v1/get-simulation-package.ts` that reads from R2 using `buildSimulationPackageKey(runId, generatedAt)`. Follows the same pattern as the deep-snapshot replay handler.
...
- [ ] `getSimulationPackage(runId)` RPC handler exists in `server/argus/forecast/v1/`
...
- New file needed: `server/argus/forecast/v1/get-simulation-package.ts`
- Wire in: `server/argus/handler.ts` (gateway registration)
- Follow pattern of: `server/argus/forecast/v1/get-forecasts.ts`
