...
const TOOL_REGISTRY: ToolDef[] = [
  {
    ...
    description: 'Real-time equity quotes, commodity prices (including gold futures GC=F), crypto prices, forex FX rates (USD/EUR, USD/JPY etc.), sector performance, ETF flows, and Gulf market quotes from Argus\'s curated bootstrap cache.',
  ...
  },
  {
    ...
    description: 'AI-classified geopolitical threat news summaries, GDELT intelligence signals, cross-source signals, and security advisories from Argus\'s intelligence layer.',
    inputSchema: { type: 'object', properties: {}, required: [] },
    _cacheKeys: [
      'news:insights:v1',
  ...
  },
  {
    ...
    description: 'Climate anomalies, NOAA atmospheric greenhouse gas monitoring (CO2 ppm, methane ppb, N2O ppb, Mauna Loa 12-month trend), weather alerts, and natural environmental events from Argus climate feeds.',
    inputSchema: { type: 'object', properties: {}, required: [] },
  ...
  },
  {
    ...
    description: 'AI-generated geopolitical and economic forecasts from Argus\'s predictive models. Covers upcoming risk events and probability assessments.',
    inputSchema: { type: 'object', properties: {}, required: [] },
  ...
  // -------------------------------------------------------------------------
  {
    ...
    _execute: async (params, base, apiKey) => {
      ...
      const digestRes = await fetch(`${base}/api/news/v1/list-feed-digest?variant=geo&lang=en`, {
        headers: { 'X-Argus-Key': apiKey, 'User-Agent': UA },
      ...
      const briefRes = await fetch(`${base}/api/news/v1/summarize-article`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Argus-Key': apiKey, 'User-Agent': UA },
        body: JSON.stringify({
          provider: 'openrouter',
          headlines,
          mode: 'brief',
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      ...
      const res = await fetch(
        ...
        {
          headers: { 'X-Argus-Key': apiKey, 'User-Agent': 'argus-mcp-edge/1.0' },
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      ...
      const UA = 'argus-mcp-edge/1.0';
      const headers = { 'X-Argus-Key': apiKey, 'User-Agent': UA };
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      ...
      const [sw_lat, sw_lon, ne_lat, ne_lon] = bbox;
      const bboxQ = `sw_lat=${sw_lat}&sw_lon=${sw_lon}&ne_lat=${ne_lat}&ne_lon=${ne_lon}`;
      const headers = { 'X-Argus-Key': apiKey, 'User-Agent': 'argus-mcp-edge/1.0' };
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      const res = await fetch(`${base}/api/intelligence/v1/deduct-situation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Argus-Key': apiKey, 'User-Agent': 'argus-mcp-edge/1.0' },
...
export default async function handler(req: Request): Promise<Response> {
  ...
  //   2. X-Argus-Key header — direct API key (curl, custom integrations)
  let apiKey = '';
  ...
  if (authHeader.startsWith('Bearer ')) {
    ...
    if (bearerApiKey) {
      ...
      return new Response(
        ...
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="argus", error="invalid_token", resource_metadata="https://api.argus-intel.vercel.app/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
  } else {
    const candidateKey = req.headers.get('X-Argus-Key') ?? '';
    if (!candidateKey) {
      return new Response(
        JSON.stringify({ jsonrpc: '2.0', id: null, error: { code: -32001, message: 'Authentication required. Use OAuth (/oauth/token) or pass your API key via X-Argus-Key header.' } }),
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="argus", resource_metadata="https://api.argus-intel.vercel.app/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
