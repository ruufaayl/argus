...
const BROWSER_ORIGIN_PATTERNS = [
  /^https:\/\/(.*\.)?argus\.app$/,
  /^https:\/\/argus-[a-z0-9-]+-elie-[a-z0-9]+\.vercel\.app$/,
...
export function validateApiKey(req, options = {}) {
  ...
  const key = req.headers.get('X-Argus-Key');
  ...
  // Trusted browser origin (argus-intel.vercel.app, Vercel previews, localhost dev) — no key needed
