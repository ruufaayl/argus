 ...
 *   2. Browser tester key (X-Argus-Key) — validated against
...
const RELAY_BASE = 'https://proxy.argus-intel.vercel.app';
...
function hasValidArgusKey(key: string): boolean {
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Argus-Key, X-Widget-Key, X-Pro-Key',
      },
    });
  }
  ...
  const worldMonitorKey = req.headers.get('X-Argus-Key') ?? '';
  if (hasValidArgusKey(worldMonitorKey)) {
    isPro = true;
  } else {
  ...
  const relayHeaders: Record<string, string> = {
    ...
    'User-Agent': 'argus-widget-edge/1.0',
