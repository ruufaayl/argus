...
- CDN-Cache-Control header now only set for trusted origins (argus.app, Vercel previews, Tauri); no-origin server-side requests always reach the edge function so `validateApiKey` can run, closing a potential cache-bypass path for external scrapers
...
- **Edge caching**: Cloudflare edge caching for proxy.argus.app (#478) and api.argus.app (#471)
...
- **Finance variant**: Added a dedicated market-first variant (`finance.argus.app`) with finance/trading-focused feeds, panels, and map defaults
...
- **CORS hardening**: Tighten Vercel preview deployment regex to block origin spoofing (`argusEVIL.vercel.app`)
