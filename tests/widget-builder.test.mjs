...
describe('widget-agent relay — security', () => {
  ...
  it('CORS reuses getCorsOrigin (not a narrow hardcoded origin list)', () => {
    ...
    assert.ok(
      !corsBlock.includes("['https://argus.app'"),
...
describe('widget-store — constants and logic', () => {
  ...
  it('deleteWidget cleans argus-panel-spans (aggregate map)', () => {
    assert.ok(
      store.includes("'argus-panel-spans'"),
      "deleteWidget must clean 'argus-panel-spans'",
    );
  });

  it('deleteWidget cleans argus-panel-col-spans (aggregate map)', () => {
    assert.ok(
      store.includes("'argus-panel-col-spans'"),
      "deleteWidget must clean 'argus-panel-col-spans'",
    );
  });
...
describe('proxy routing — widgetAgentUrl', () => {
  ...
  it('vite.config.ts proxies /widget-agent to proxy.argus.app', () => {
    assert.ok(
      vite.includes('/widget-agent'),
      'vite.config.ts must have proxy entry for /widget-agent',
    );
    assert.ok(
      vite.includes('proxy.argus.app'),
      'Vite proxy target must be proxy.argus.app',
    );
  });
