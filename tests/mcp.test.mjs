...
const BASE_URL = 'https://api.argus.app/mcp';
...
describe('api/mcp.ts — PRO MCP Server', () => {
  ...
  it('returns HTTP 401 + WWW-Authenticate when no credentials provided', async () => {
    ...
    assert.ok(res.headers.get('www-authenticate')?.includes('Bearer realm="argus"'), 'must include WWW-Authenticate header');
    const body = await res.json();
    assert.equal(body.error?.code, -32001);
  });

  it('returns JSON-RPC -32001 when invalid API key provided', async () => {
    const req = makeReq('POST', initBody(), { 'X-Argus-Key': 'wrong_key' });
    const res = await handler(req);
  ...
  it('OPTIONS returns 204 with CORS headers', async () => {
    const req = new Request(BASE_URL, { method: 'OPTIONS', headers: { origin: 'https://argus.app' } });
    const res = await handler(req);
  ...
  it('initialize returns protocol version and Mcp-Session-Id header', async () => {
    ...
    assert.equal(body.result?.serverInfo?.name, 'argus');
  ...
  it('malformed body returns JSON-RPC -32600', async () => {
    const req = new Request(BASE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Argus-Key': VALID_KEY },
      body: '{bad json',
    });
    const res = await handler(req);
    const body = await res.json();
