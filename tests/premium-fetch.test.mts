...
describe('premiumFetch', () => {
  ...
  it('passthrough when Authorization header already set', async () => {
    ...
    assert.equal(sentHeaders().get('X-Argus-Key'), null);
  });

  it('passthrough when X-Argus-Key header already set', async () => {
    setup();
    await premiumFetch(TARGET, { headers: { 'X-Argus-Key': 'caller-key' } });
    assert.equal(fetchMock.mock.calls.length, 1);
    assert.equal(sentHeaders().get('X-Argus-Key'), 'caller-key');
  });
  ...
  it('tester key: valid key accepted — exactly one fetch, key forwarded', async () => {
    ...
    assert.equal(sentHeaders().get('X-Argus-Key'), 'valid-gateway-key');
  });
  ...
  it('tester key: 401 falls through to Clerk JWT (two fetches)', async () => {
    ...
    assert.equal(sentHeaders(0).get('X-Argus-Key'), 'widget-only-key');
    ...
    assert.equal(sentHeaders(1).get('X-Argus-Key'), null);
  });
  ...
  it('wm-pro-key 401 retries with wm-widget-key before Clerk', async () => {
    ...
    assert.equal(sentHeaders(0).get('X-Argus-Key'), 'relay-only-pro-key');
    assert.equal(sentHeaders(0).get('Authorization'), null);
    assert.equal(sentHeaders(1).get('X-Argus-Key'), 'valid-widget-key');
