...
describe('getHumanitarianSummary handler', () => {
  const src = readSrc('server/argus/conflict/v1/get-humanitarian-summary.ts');
...
describe('LLM prompt political context (LOW-1)', () => {
  const src = readSrc('server/argus/news/v1/_shared.ts');
...
describe('getCacheKey determinism', () => {
  ...
  const sharedSrc = readSrc('server/argus/news/v1/_shared.ts');
...
describe('getVesselSnapshot caching (HIGH-1)', () => {
  const src = readSrc('server/argus/maritime/v1/get-vessel-snapshot.ts');
...
describe('getSimulationOutcome handler', () => {
  const src = readSrc('server/argus/forecast/v1/get-simulation-outcome.ts');
