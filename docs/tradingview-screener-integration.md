...
**Purpose:** Reference document for extending Argus's finance hub with TradingView Screener data.
...
The TypeScript library is the one relevant for Argus. It has full parity with the Python version (41/41 operations pass) plus a self-hosted REST server option.
...
Argus's data flow: `Railway seeds Redis → Vercel reads Redis → Frontend RPC`
...
```typescript
// server/argus/market/v1/list-tv-stock-screener.ts
...
### Argus-Specific Recommendations
...
| **No auth initially** | Delayed data is fine for Argus's use case |
...
- [ ] Create `proto/argus/market/v1/list_tv_stock_screener.proto`
- [ ] Create `server/argus/market/v1/list-tv-stock-screener.ts`
