# ARGUS Pro — Implementation Roadmap
...
**Description**:
Evaluate and select an authentication provider for ARGUS Pro.
...
1. Register `argus://auth/callback` deep link URI scheme in Tauri config
...
1. **ARGUS Pro Monthly** — $X/mo
2. **ARGUS Pro Annual** — $X/yr (discount)
3. **ARGUS API Starter** — $Y/mo (1,000 req/day, 5 webhook rules)
4. **ARGUS API Business** — $Z/mo (50,000 req/day, unlimited webhooks + SLA)
...
- New sebuf proto service: `argus/equity/v1/`
...
3. **Telegram** — Bot API. Create `@ArgusBot`. User starts conversation, store `chat_id`.
...
**Description**:
Pro users should NOT need to configure individual API keys for Finnhub, FRED, ACLED, etc. A single ARGUS Pro subscription gives access to all 24 services.

**Implementation**:

- Server-side: pro requests use ARGUS's own upstream API keys (already configured as env vars)
...
5. **Desktop auth + Tauri WKWebView**: Known limitations. PKCE flow with `argus://` deep link callback.
