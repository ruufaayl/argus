...
ARGUS's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['argus-panel-spans']` as a JSON map of `{ [id]: height }`.
...
ARGUS ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['argus_recent_searches']`
(most recent 10).
