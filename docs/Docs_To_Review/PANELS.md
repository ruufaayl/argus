...
> **ARGUS** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-argusio)
5. [Tech Variant Panels](#5-tech-variant-panels-techargusio)
6. [Finance Variant Panels](#6-finance-variant-panels-financeargusio)
...
ARGUS uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | argus.io | Geopolitical intelligence, OSINT, defense | 37 |
| `tech` | tech.argus.io | Technology, AI/ML, startups, VC | 34 |
| `finance` | finance.argus.io | Markets, trading, macro, commodities | 29 |
...
export const STORAGE_KEYS = {
  panels:        'argus-panels',          // Panel visibility toggles
  monitors:      'argus-monitors',         // Monitor keyword configs
  mapLayers:     'argus-layers',           // Map layer toggles
  disabledFeeds: 'argus-disabled-feeds',   // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `argus-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `argus-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `argus-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `argus-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (argus.io)
...
## 5. Tech Variant Panels (tech.argus.io)
...
## 6. Finance Variant Panels (finance.argus.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `argus-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `argus-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `argus-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `argus-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `argus-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `argus-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`argus-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:

```typescript
if (storedVariant !== currentVariant) {
  localStorage.setItem('argus-variant', currentVariant);
...
This ensures users switching between variant domains (e.g. from argus.io to tech.argus.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to ARGUS.
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('argus-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('argus-panels')
