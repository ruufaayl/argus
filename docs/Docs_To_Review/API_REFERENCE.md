...
> Comprehensive reference for all Vercel Edge Function endpoints powering the ARGUS intelligence dashboard.

**Base URL**: All endpoints are relative to `/api/` (e.g., `https://argus.app/api/earthquakes`).
...
ARGUS exposes **60+ serverless endpoints** deployed as **Vercel Edge Functions** (unless noted otherwise). Every endpoint:
...
| # | Pattern | Matches |
|---|---------|---------|
| 1 | `argus\.app$` | `https://argus.app` |
| 2 | `\.argus\.app$` | `https://*.argus.app` |
...
ARGUS uses a **multi-tier caching strategy** to minimize upstream API calls, reduce latency, and stay within third-party rate limits.
