...
ARGUS is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
```typescript
const PANEL_SPANS_KEY = 'argus-panel-spans';
...
Stored in `localStorage` under `argus-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `argus-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `argus-monitors` | `Monitor[]` | Color/label configs for monitors |
| `argus-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `argus-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('argus-theme', 'light')
...
- **Toggles key:** `argus-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by ARGUS:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `argus-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `argus-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `argus-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `argus-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `argus-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `argus-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
| `argus-disabled-feeds` | User-disabled news sources | `string[]` (JSON) | `STORAGE_KEYS.disabledFeeds` |
| `argus-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `argus-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `argus-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `argus-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
