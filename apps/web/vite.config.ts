...
function argusApiMiddleware(): Plugin {
  ...
  const syncNews = async () => {
    ...
    isSyncing = true;
    try {
      // 1. Fetch Latest Articles via GDELT (Pure JSON, no scrapper libraries)
      ...
      const headlines = (rawData.articles || []).slice(0, 10).map((a: any) => a.title);
      ...
      if (startIdx !== -1 && endIdx !== -1) {
        newsCache = JSON.parse(content.substring(startIdx, endIdx + 1));
  ...
  // Immediate sync on load and every 60s
  setTimeout(syncNews, 500);
  setInterval(syncNews, 60 * 1000);
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/intel/signals', (req, res) => {
        ...
        if (newsCache.length > 0) {
          ...
          // Passive revalidation: if older than 60s, trigger background sync
