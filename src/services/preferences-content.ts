...
const DESKTOP_RELEASES_URL = 'https://github.com/ruufaayl/argus/releases';
    ...
    attach(container: HTMLElement): () => void {
      ...
      if (!host.isDesktopApp && host.isSignedIn) {
        ...
        container.addEventListener('click', (e) => {
          ...
          const startTelegramPairing = (rowEl: HTMLElement) => {
            ...
            createPairingToken().then(({ token, expiresAt }) => {
              if (signal.aborted) return;
              const botUsername = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TELEGRAM_BOT_USERNAME as string | undefined) ?? 'ArgusBot';
        ...
        const onMessage = (e: MessageEvent): void => {
          // Bind trust to both: (1) a WM-owned origin (callback is always on argus.app,
          // but settings may be open on a different *.argus.app subdomain) and
          ...
          const trustedOrigin = e.origin === window.location.origin ||
            e.origin === 'https://argus.app' ||
            e.origin === 'https://www.argus.app' ||
            e.origin.endsWith('.argus.app');
