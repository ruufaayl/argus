...
// Cookies with domain=.argus-intel.vercel.app are shared across all subdomains
// (argus-intel.vercel.app, tech., finance., commodity., happy., etc.).
// We read cookie first and fall back to localStorage for migration compat.

const COOKIE_DOMAIN = '.argus-intel.vercel.app';
const KEY_MAX_AGE = 365 * 24 * 60 * 60;

function usesCookies(): boolean {
  return location.hostname.endsWith('argus-intel.vercel.app');
}
