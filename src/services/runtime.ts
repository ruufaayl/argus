...
const DEFAULT_WEB_API_URL = 'https://api.argus.app';
...
function isArgusWebHost(hostname: string): boolean {
  return hostname === 'argus.app'
    || hostname === 'www.argus.app'
    || hostname.endsWith('.argus.app');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://argus.app';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.argus.app');
  } catch {
    return false;
  }
}
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        if (wmKeyValue) {
          cloudHeaders.set('X-Argus-Key', wmKeyValue);
        }
      }
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*argus\.app(:\d+)?$/;
...
export function installWebApiRedirect(): void {
   ...
   *   2. WORLDMONITOR_API_KEY from runtime config → X-Argus-Key
   *   3. Tester key (wm-pro-key / wm-widget-key) → X-Argus-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (testerKey) {
      headers.set('X-Argus-Key', testerKey);
      return { ...init, headers };
    }
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.argus.app/api/...) — just inject auth.
