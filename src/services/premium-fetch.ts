...
function reportServerError(res: Response, input: RequestInfo | URL): void {
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://argus.app').pathname;
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-Argus-Key')) {
  ...
  try {
    ...
    if (wmKey) {
      existing.set('X-Argus-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-Argus-Key', testerKey);
