...
const PANEL_COL_SPANS_KEY = 'argus-panel-col-spans';
...
const PANEL_COLLAPSED_KEY = 'argus-panel-collapsed';
...
export class Panel {
  ...
  public showLocked(features: string[] = []): void {
    ...
    if (isDesktopRuntime()) {
      ctaBtn.addEventListener('click', () => void invokeTauri<void>('open_url', { url: 'https://argus.app/pro' }).catch(() => window.open('https://argus.app/pro', '_blank')));
    } else {
      ctaBtn.addEventListener('click', () => window.open('https://argus.app/pro', '_blank'));
    }
