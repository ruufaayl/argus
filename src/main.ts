...
Sentry.init({
  ...
  release: `argus@${__APP_VERSION__}`,
  environment: (location.hostname === 'argus.app' || location.hostname.endsWith('.argus.app')) ? 'production'
    : location.hostname.includes('vercel.app') ? 'preview'
    : 'development',
  enabled: Boolean(sentryDsn) && !location.hostname.startsWith('localhost') && !('__TAURI_INTERNALS__' in window),
  allowUrls: [
    /https?:\/\/(www\.|tech\.|finance\.|commodity\.|happy\.)?argus\.app/,
    /https?:\/\/.*\.vercel\.app/,
  ],
  sendDefaultPii: true,
...
// In web production, route RPC calls through api.argus.app (Cloudflare edge).
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('argus-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('argus-beta-mode', 'true');
    else localStorage.removeItem('argus-beta-mode');
    location.reload();
  },
});
