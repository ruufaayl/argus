...
import type { ListFeedDigestResponse } from '@/generated/client/argus/news/v1/service_client';
import type { GetSectorSummaryResponse, ListMarketQuotesResponse, ListCommodityQuotesResponse } from '@/generated/client/argus/market/v1/service_client';
import { mountCommunityWidget } from '@/components/CommunityWidget';
import { ResearchServiceClient } from '@/generated/client/argus/research/v1/service_client';
import {
  MarketPanel,
  StockAnalysisPanel,
  StockBacktestPanel,
...
import type { NewsItem as ProtoNewsItem, ThreatLevel as ProtoThreatLevel } from '@/generated/client/argus/news/v1/service_client';
...
export class DataLoaderManager implements AppModule {
  ...
  private isPanelNearViewport(panelId: string, marginPx = 400): boolean {
    ...
    const el = (panel as unknown as { getElement?: () => HTMLElement })?.getElement?.();
    if (el && el.closest && el.closest('#mapBottomGrid')) {
      // Panels docked under the map should load immediately even if just offscreen.
      return true;
    }
  ...
  async loadAllData(forceAll = false): Promise<void> {
    ...
    if (SITE_VARIANT !== 'happy' && this.ctx.mapLayers.satellites) tasks.push({ name: 'satellites', task: runGuarded('satellites', () => this.loadSatellites()) });
  ...
  private async _collectRegimeContext(): Promise<RegimeMacroContext | undefined> {
    try {
      ...
      const { MarketServiceClient } = await import('@/generated/client/argus/market/v1/service_client');
  ...
  private async _collectYieldCurveContext(): Promise<YieldCurveContext | undefined> {
    try {
      const { EconomicServiceClient } = await import('@/generated/client/argus/economic/v1/service_client');
      const { getRpcBaseUrl } = await import('@/services/rpc-client');
  ...
  async loadForecasts(): Promise<void> {
    try {
      const hydrated = getHydratedData('forecasts') as { predictions?: import('@/generated/client/argus/forecast/v1/service_client').Forecast[] } | undefined;
  ...
  async loadIntelligenceSignals(): Promise<void> {
    ...
    const hydratedUcdp = getHydratedData('ucdpEvents') as import('@/generated/client/argus/conflict/v1/service_client').ListUcdpEventsResponse | undefined;
  ...
  async loadEconomicStress(): Promise<void> {
    try {
      ...
      const hydrated = getHydratedData('economicStress') as import('@/generated/client/argus/economic/v1/service_client').GetEconomicStressResponse | undefined;
